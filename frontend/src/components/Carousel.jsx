...
const Carousel = ({ items }) => {
    ...
    }, [currentIndex]); // Add dependency array to prevent infinite loop
    ...
    return (
        <div className="relative overflow-hidden w-full">
            ...
            >
                {items.map((item, index) => (
                    ...
                    >
                        ...
                        <img src={item} alt={`Carousel item ${index}`} className="w-full h-full object-cover" />
                    </div>
                ))}
            </div>
            ...
            <button onClick={handleNext} className="absolute right-0 top-1/2 transform -translate-y-1/2 p-2 bg-gray-800/50 hover:bg-gray-800/70 focus:outline-none focus:ring-2 focus:ring-white rounded-full">
                <svg
                    className="w-6 h-6 text-white"
                    fill="none"
                    viewBox="0 0 24 24"
