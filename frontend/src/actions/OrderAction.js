...
export const getOrderDetails = (orderId) => async (dispatch) => {
    try {
        ...
        axios.defaults.withCredentials = true;
        const { data } = await axios.get(`http://localhost:8080/api/v1/order/${orderId}`);
...
export const createOrder = (orderData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(`http://localhost:8080/api/v1/order/new`, orderData, config);
...
export const cancelOrder = (orderId, newStatus) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:8080/api/v1/order/${orderId}/cancel`, {newStatus}, config);
...
export const deleteOrderAdmin = (orderId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`http://localhost:8080/api/v1/admin/order/${orderId}/delete`, config);
...
export const updateOrderAdmin = (orderId, orderData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:8080/api/v1/admin/order/${orderId}/update`, {orderData}, config);
